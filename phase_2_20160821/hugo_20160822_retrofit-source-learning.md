...
- [Retrofit 分析](#retrofit-分析)
	- [具体使用](#具体使用)
	- [工具箱：Retrofit.Builder()](#工具箱retrofitbuilder)
	- [支架：Create()](#支架create)
	- [外壳：ServiceMethod](#外壳servicemethod)
	- [子弹：xxxFactory()](#子弹xxxfactory)
	- [开枪打靶: Call.enqueue()](#开枪打靶-callenqueue)
- [参考](#参考)
...
`GankApi gankApi = retrofit.create(GankApi.class);`—->
 `return (T) Proxy.newProxyInstance（...）{...}`注意：这时候不会进入 invoke()  —->
 `Call<Android> call = gankApi.getAndroid(1);` —->
...
```
我们发现主要的方法是 `new ServiceMethod.Builder(this, method).build();` ，所以接下来我们深入看看如何 __解析注解__ 遗迹 __构建请求方法__ 。
...
Retrofit 给我们最大的便利就是自身框架优雅的设计，只需要很小的改动，便可以优雅的适应不同的需求。
...
这里的 Call 是对应我们选择的 call ，而此时是默认的 `ExecutorCallbackCall` 。如果还要问我为什么，请去看看[工具箱：Retrofit.Builder()](#工具箱retrofitbuilder) 里 Android 平台的源码。

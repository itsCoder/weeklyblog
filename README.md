...
- 开发规范：
  ...
  - 基于每次 pr 进行审阅，提出修改或者有疑问的评论，审阅完毕给出评价

  - 作者根据审核意见进行修改，修改完成后，标记上 **Done** 标签，同时给出自己博客上的地址，格式如下：

    ```
    [文章标题](文章链接) ([@作者名](作者主页地址，可以为 GitHub 地址)
    ```

  - 完成审阅后由负责人合并到 dev 分支，每期结束时 dev 分支合并到 master 分支

  - 每个成员只允许在自己成员分支上进行操作

- 审阅规范：

  - 审阅人员优先认领，后续再通过其他方式指定，每篇文章保证有人审阅
